...
透過 LLM + XML 解析器 + CSV 儲存器，判斷簡訊是否包含中文姓名
並將分類結果存至指定檔案格式：name_{model_name}_{yyyyMMdd_hhmm}.csv
...
class NameClassifier:
    ...
    def _load_prompt(self) -> str:
        prompt_paths = [
            ...
            "name_classifier_prompt.md",
            Path(__file__).parent / "name_classifier_prompt.md"
        ]
    ...
    def process_file(self, input_path: str, output_dir: str = None) -> str:
        ...
        existing_ids = get_existing_classified_ids(
            output_dir, "name", self.model_name)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M")
        safe_model_name = self.model_name.replace("/", "_").replace(":", "_")
        output_filename = f"name_{safe_model_name}_{timestamp}.csv"
