...
# -*- coding: utf-8 -*-
"""
Distilled Multilingual BERT 旅遊簡訊分類器
整合式分類器 - 處理 stage 1 及 stage 2 的旅遊分類任務

這個模組提供完整的 BERT 旅遊分類功能，包括：
1. 模型訓練 (TravelModelTrainer)
2. 模型推論 (TravelBertInference) 
3. 競賽提交檔案生成
4. 閾值優化

使用方式:
    # 訓練模型
    python -m src.bert_travel_classifier train --labeled_csv path/to/labeled.csv --input_csv path/to/input.csv
    
    # 進行推論
    python -m src.bert_travel_classifier predict --model_path path/to/model --input_csv path/to/input.csv
    
    # 生成提交檔案
    python -m src.bert_travel_classifier submit --model_path path/to/model --input_csv path/to/input.csv --threshold 0.7
"""
...
try:
    ...
    from bert_model.bert_travel_inference import TravelBertInference, generate_submission, optimize_threshold_on_validation
except ImportError:
    # 直接執行時的匯入方式
    from bert_travel_trainer import TravelModelTrainer
    from bert_travel_inference import TravelBertInference, generate_submission, optimize_threshold_on_validation


class TravelBertClassifier:
    """
    Distilled Multilingual BERT 旅遊分類器主類別
    整合訓練和推論功能
    """
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        if model_path and os.path.exists(model_path):
            self.inferencer = TravelBertInference(model_path)
    ...
    def train(
        ...
        **kwargs
        ...
        # 初始化訓練器
        self.trainer = TravelModelTrainer(
            use_wandb=use_wandb,
    ...
    def generate_submission(
        ...
        output_dir: str = "data_game_2025/data/results"
    ) -> str:
        """
        生成競賽提交檔案
        
        Args:
            input_csv_path: 輸入簡訊檔案
            threshold: 分類閾值
    ...
    def optimize_threshold(
        ...
        return optimize_threshold_on_validation(
            self.model_path,
            validation_csv,
            input_csv
        )
...
def main():
    """命令列介面"""
    parser = argparse.ArgumentParser(description="BERT 旅遊簡訊分類器")
    ...
    train_parser.add_argument('--output_dir', default='data_game_2025/data/results/travel_bert', help='模型輸出目錄')
    ...
    train_parser.add_argument('--wandb_project', default='bert-travel-classifier', help='wandb 專案名稱')
    ...
    if args.command == 'train':
        ...
        classifier = TravelBertClassifier(args.model_path)
